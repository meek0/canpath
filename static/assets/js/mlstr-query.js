...
class MicaQueryExecutor {
  ...
  /**
   * Compares current and new URL to determine weather the new URL should replace the current or to be added to history.
   *
   * @param tree
   * @param type
   * @param display
   * @returns {boolean} true implies that the URL history should be replaced.
   * @private
   */
  __shouldUpdateUrl(tree, type, display) {
    const target = TYPES_TARGETS_MAP[type];
    const parsedUrl = MicaTreeQueryUrl.parseUrl();
    const current = MicaTreeQueryUrl.getTree(parsedUrl);
    const currentDisplay = parsedUrl.hash;

    if (current && current._root.args.length > 0) {
      const currentlimitQuery = current.search((name, args, parent) => parent && parent.name === target && name === 'limit');
      let cloned = new RQL.QueryTree(RQL.Parser.parseQuery(tree.serialize()), QueryTreeOptions);

      if (!currentlimitQuery) {
        const targetQuery = cloned.search(name => target === name);
        if (targetQuery && (targetQuery.args || []).length > 0) {
          cloned.findAndDeleteQuery((name) => 'limit' === name);
        }
      }

      return cloned.serialize() === current.serialize() && display === currentDisplay;
    }
  ...
  __prepareAndExecuteQuery(eventId, payload) {
    ...
    switch (type) {
      ...
      case TYPES.NETWORKS:
        ...
        if (tree) {
          const noUrlUpdate = payload.noUrlUpdate || this.__shouldUpdateUrl(tree, type, display);
          switch (display) {
            case DISPLAYS.COVERAGE:
              const coverageTree = entityQuery.prepareForCoverage(tree, bucket);
              if (!coverageTree) {
                this.__ignoreCoverage(tree, type, display, noUrlUpdate, bucket);
              } else {
                this.__executeCoverage(coverageTree, type, display, noUrlUpdate, bucket);
              }
              break;

            case DISPLAYS.LISTS:
              this.__executeQuery(entityQuery.prepareForQuery(type, tree), type, display, noUrlUpdate);
              break;
